...
export default function TopNav({ onOpenCollection }) {
  ...
  return (
    <>
      <audio ref={audioRef} loop autoPlay src={asset("/audio/champ-select.mp3")} />
      ...
      <nav className="flex items-center justify-between bg-linear-to-b from-lol-nav-top to-lol-nav-bottom border-b border-lol-border px-5 h-12 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-1.5">
          <img src={asset("/images/icons/poro.gif")} alt="LoL" className="w-8 h-8" />
          {tabs.map((tab) => (
            <button
              key={tab.label}
        ...
        <div className="flex items-center gap-4 text-sm text-lol-grey">
          <span>LOOT</span>
          <span>STORE</span>
          <span className="flex items-center gap-0.5"><img src={asset("/images/icons/currency.jpg")} className="w-4 h-4" /> 446</span>
          <span className="flex items-center gap-0.5"><img src={asset("/images/icons/blue-essence.jpg")} className="w-4 h-4" /> 6444</span>
          <button
            onClick={toggleMute}
